...
TEST (votes, add_existing)
{
	...
	rai::genesis genesis;
	rai::keypair key1;
	auto send1 (std::make_shared<rai::send_block> (genesis.hash (), key1.pub, rai::genesis_amount - rai::kBAN_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0));
	{
		auto transaction (node1.store.tx_begin (true));
		ASSERT_EQ (rai::process_result::progress, node1.ledger.process (transaction, *send1).code);
	}
	...
	rai::keypair key2;
	auto send2 (std::make_shared<rai::send_block> (genesis.hash (), key2.pub, rai::genesis_amount - rai::kBAN_ratio, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0));
...
TEST (ledger, epoch_blocks_receive_upgrade)
{
	...
	rai::receive_block receive1 (open1.hash (), send2.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (rai::process_result::unreceivable, ledger.process (transaction, receive1).code);
	rai::state_block receive2 (destination.pub, open1.hash (), destination.pub, rai::kBAN_ratio * 2, send2.hash (), destination.prv, destination.pub, 0);
	ASSERT_EQ (rai::process_result::progress, ledger.process (transaction, receive2).code);
	...
	rai::state_block send3 (destination.pub, receive2.hash (), destination.pub, rai::kBAN_ratio, destination2.pub, destination.prv, destination.pub, 0);
